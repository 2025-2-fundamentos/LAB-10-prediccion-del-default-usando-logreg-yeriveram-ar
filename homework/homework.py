...
def main():
    path1 = "./files/input/"
    path2 = "./files/models/"
    path_output = "./files/output/" # 🌟 Nueva variable para la ruta de salida

    # 1. Crear el directorio de modelos si no existe
    if not os.path.exists(path2):
        os.makedirs(path2)
        print(f"Directorio de modelos creado: {path2}")
        
    # 2. 🌟 Crear el directorio de salida si no existe
    if not os.path.exists(path_output):
        os.makedirs(path_output)
        print(f"Directorio de salida creado: {path_output}")
